...
if __name__ == '__main__':
    ...
    db.commit()
    sql_max_dt = "select max(b.state_dt) from future_tick b where b.future_code = '%s'"%(future_code)
    cursor.execute(sql_max_dt)
    done_set_max = cursor.fetchall()
    db.commit()
    if len(done_set_max) > 0:
        sql_delete = "delete a.* from future_tick a where a.state_dt = '%s' and a.future_code = '%s'" %(str(done_set_max[0][0]),future_code)
    ...
    for i in range(len(date_seq)):
        sql_check = "select * from future_tick a where a.state_dt = '%s' and a.future_code = '%s'"%(date_seq[i],future_code)
        ...
        if c_len > 0:
            for j in range(c_len):
                ...
                sql_select = "select * from future_tick a where a.future_code = '%s' and a.state_time = '%s'"%(future_code,state_time)
                cursor.execute(sql_select)
                done_set_select = cursor.fetchall()
                db.commit()
                if len(done_set_select) > 0:
                    sql_update = "update future_tick w set w.future_vol = w.future_vol + %i"%(future_vol)
                    cursor.execute(sql_update)
                    db.commit()
                else:
                    sql_insert = "insert into future_tick(state_dt,state_time,future_code,future_price,future_vol,deal_type,state_ts)values('%s','%s','%s','%.2f','%i','%s','%i')"%(state_dt,state_time,future_code,future_price,future_vol,deal_type,state_ts)
